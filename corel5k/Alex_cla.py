...
#    loss_val.append((sum_sce+sum_term)/N)
    ...
    with open('./loss_data/adam/eigenvector/origin_train_%.6f_%.2f_%d.csv'%(rate,weight_p,dim),'ab') as csvfile:
...
#    loss_val.append((sum_sce+sum_term)/N)
    ...
    with open('./loss_data/adam/eigenvector/origin_test_%.6f_%.2f_%d.csv'%(rate,weight_p,dim),'ab') as csvfile:
        writer=csv.writer(csvfile)
        writer.writerow(loss_val)
model.to_cpu()

serializers.save_npz('./model/model_%.6f_%.2f_%d.model'%(rate,weight_p,dim),model)
